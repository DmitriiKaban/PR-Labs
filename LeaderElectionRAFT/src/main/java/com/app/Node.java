...
//            startElectionTimer();
    ...
    private void resetHeartbeatTimeout() {
        heartbeatTimeoutExecutor.schedule(() -> {
            if (!leaderAlive.get() && currentState != State.LEADER) {
                System.out.println("Node " + nodeId + ": Heartbeat timeout! Initiating election...");
                startOrRestartHeartbeatTimeout();
                startElection();
            } else {
                leaderAlive.set(false); // Reset for the next heartbeat
            }
